...
TH1D* HIST_qq=0; // qqbar signal histogram
TH1D* HISTCDF_qq=0; // qqbar signal CDF
...
double INTEGRAL_0Tag(double *fx0, double *fxf, double *par)
{
  ...
  int bin1=HISTCDF_qq->GetXaxis()->FindBin(xprimef);
  int bin2=HISTCDF_qq->GetXaxis()->FindBin(xprime0);
  if(bin1<1) bin1=1;
  if(bin1>HISTCDF_qq->GetNbinsX()) bin1=HISTCDF_qq->GetNbinsX();
  if(bin2<1) bin1=1;
  if(bin2>HISTCDF_qq->GetNbinsX()) bin2=HISTCDF_qq->GetNbinsX();
  double sig=xs*eff*lumi*(HISTCDF_qq->GetBinContent(bin1)-HISTCDF_qq->GetBinContent(bin2));
...
int main(int argc, char* argv[])
{
  ...
  TFile* histfile_gg=new TFile("Data_and_ResonanceShapes/Resonance_Shapes_WideJets_qq.root");

  histname_gg << "h_qq_" << masspoint;
  cdfname_gg << "h_qq_" << masspoint << "_cdf";

  HIST_qq=(TH1D*)histfile_gg->Get(histname_gg.str().c_str());
  HISTCDF_qq=(TH1D*)histfile_gg->Get(cdfname_gg.str().c_str());

  assert(HIST_qq && HISTCDF_qq && SIGMASS>0);
